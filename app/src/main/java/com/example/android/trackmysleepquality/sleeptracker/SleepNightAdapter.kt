...
class SleepNightAdapter(val clickListener: SleepNightListener) : ListAdapter<DataItem, RecyclerView.ViewHolder>(SleepNightDiffCallback()) {
    ...
    //coroutine for header list manipulation
...
sealed class DataItem {
    abstract val id: Long //SleepNightItem and Header need to override the abstract property id.
    data class SleepNightItem(val sleepNight: SleepNight): DataItem() {
        override val id = sleepNight.nightId
    }
    object Header: DataItem() {
        override val id = Long.MIN_VALUE //a very, very small number (literally, -2 to the power of 63). So, this will never conflict with any nightId in existence.
    }
}
